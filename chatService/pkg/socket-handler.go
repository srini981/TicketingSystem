...
func handleSocketPayloadEvents(client *Client, socketEventPayload SocketEventStruct) {
			...
			if userDetails.Online == "N" {
				...
				allOnlineUsersPayload := SocketEventStruct{
					...
					EventPayload: chatlistResponseStruct{
						Type:     "my-chat-list",
						Chatlist: socketEventPayload.ToUserID,
					},
				}
	...
	case "message":
		fmt.Println("payload", socketEventPayload)
		message := socketEventPayload.EventPayload.(string)
		fromUserID := socketEventPayload.FromUserID
		toUserID := socketEventPayload.ToUserID
